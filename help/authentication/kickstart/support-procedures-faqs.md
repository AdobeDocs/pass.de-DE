...
* Während des Authentifizierungsprozesses bleibt der Benutzer auf einer Adobe-Fehlerseite stecken, ohne dass der Authentifizierungsfluss erneut initiiert werden kann.
...
Adobe unterstützt Ermittlungen zu diesen Problemen, die jedoch nicht als Vorfälle der Stufe 1 eingestuft werden:
...
1. Der Partner identifiziert einen Vorfall der Stufe 1, der ein sofortiges Eingreifen Adobes erfordert.
...
1. Wenn Adobe bei der Folgenabschätzung feststellt, dass das Problem unter eine der im Voraus vereinbarten Entscheidungen über Vorfallsszenarien fällt, wird es entsprechend handeln, ohne auf die Beiträge des Partners zu warten.

1. Adobe wartet auf Updates vom Partner und eine Benachrichtigung, wenn der Service wiederhergestellt wurde.
...
| S2 | Adobe aktiviert eine neue MVPD für einen Programmierer, und der Programmierer lässt die MVPD vor dem Launch-Datum zu. | Adobe aktiviert eine neue MVPD für die Site eines Programmierers, und die Site zeigt die neue MVPD bereits in der Auswahl an, selbst wenn sie nicht vorgesehen war. | Adobe benachrichtigt den Programmierer, dass die neue MVPD vor dem geplanten Datum in der Auswahl angezeigt wird. </br></br> Programmierer wird bei Bedarf Maßnahmen ergreifen, um sie aus der Auswahl zu entfernen. |
