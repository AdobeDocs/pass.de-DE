...
* **Der temporäre Pass-Zugriff wird einzelnen Benutzern auf bestimmten Geräten gewährt.**. Nachdem der Zugriff auf den temporären Pass für einen Benutzer abläuft, kann dieser Benutzer keinen temporären Zugriff auf dasselbe Gerät erhalten, bis dessen Ablauf abgelaufen ist [Autorisierungstoken](/help/authentication/glossary.md#authz-token) wird vom Adobe Pass-Authentifizierungsserver gelöscht.
...
* **Tracking von Daten im Zeitverlauf** - Zwei Punkte bezüglich der Verfolgung von Daten während eines Berechtigungsablaufs für den temporären Pass:
   * Die Tracking-ID, die von der Adobe Pass-Authentifizierung an Ihre **sendTrackingData()** callback ist ein Hash der Geräte-ID.
...
Um alle temporären Passes zurückzusetzen, bietet die Adobe Pass-Authentifizierung Programmierern eine *öffentlich* Web-API:
